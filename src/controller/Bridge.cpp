...
void configurePort(int portNumber, JsonObject& portConfig) {
  ...
  if (postDataInterval == 0)
  {
    ...
    if (port.equals("ADC"))
    {
      ...
      {
        addSensor(new SensorAnalogue (portConfig["id"], portConfig["c"], portConfig["sn"], portConfig["n"], portConfig["c1"], 0, refreshInterval, postDataInterval, portConfig["s"]["svt"], calc));
      }
      else
      {
        addSensor(new SensorAnalogue (portConfig["id"], portConfig["c"], portConfig["sn"], portConfig["n"], 0, 0, refreshInterval, postDataInterval, portConfig["s"]["svt"], calc));
      }
    }
  #elif ESP32_BOARD
